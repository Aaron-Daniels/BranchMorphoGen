...
namespace PrintTIFF {
    void writeTIFF(const std::string& filename, const std::vector<uint16_t>& data, uint32_t height, uint32_t width, const SimulationParameters& params);
    void printImage(std::vector<Branch>& allBranches,const SimulationParameters& params, const int sample,const double& tsim);
